...
namespace seneca
{
  ...
  double Toy::getToySalesTax() const
  {
    return m_toySalesTax;
  }

  Toy &Toy::operator=(const Toy &copyFromToy)
  {
    if(this == &copyFromToy)
      return *this;
    
    m_toyOrderId = copyFromToy.getToyId();
    m_toyName = copyFromToy.getToyName();
    m_toyOrders = copyFromToy.getToyOrders();
    m_toyPrice = copyFromToy.getToyPrice();
    m_toySalesTax = copyFromToy.getToySalesTax();

    return *this;
  }

  Toy::Toy(const Toy &copyFromToy)
  {
    m_toyOrderId = copyFromToy.getToyId();
    m_toyName = copyFromToy.getToyName();
    m_toyOrders = copyFromToy.getToyOrders();
